...
inputDocuments:
  - _bmad-output/planning-artifacts/healthcare-platform-complete-prd/definition/prd.md
  - _bmad-output/planning-artifacts/healthcare-platform-complete-prd/solution-design/overall-lld.md
---

# Healthcare Platform — Complete Feature Gap: Epic Breakdown

## Overview

This document decomposes all 175 stories from the Complete Feature Gap PRD (`../definition/prd.md`) into 12 epics, one per persona plus a cross-cutting epic, with the full Given/When/Then acceptance criteria carried over from the PRD. Architecture inputs are drawn from the Overall Low-Level Design (`../solution-design/overall-lld.md`).

## Requirements Inventory

### Functional Requirements

- **FR-TR-001**: As a triage nurse, I want to assign a Canadian Triage and Acuity Scale (CTAS) level (1–5) to each arriving patient, so that clinical staff can prioritise care appropriately and meet legislated wait-time targets.
- **FR-TR-002**: As a triage nurse, I want to record a structured chief complaint with body system, onset, severity, and duration, so that the clinical team has actionable, coded reason-for-visit data rather than free text.
- **FR-TR-003**: As a triage nurse, I want to record a complete vital signs set including weight, height, BMI, temperature, BP, HR, SpO2, RR, and pain score, so that clinicians have a full baseline and calculated derived values are correct.
- **FR-TR-004**: As a triage nurse, I want to record new allergies and verify or update existing ones during triage, so that clinicians and pharmacists receive accurate allergy information before any medications are ordered or dispensed.
- **FR-TR-005**: As a triage nurse, I want to complete a standardised fall risk assessment (Morse Fall Scale) for eligible patients, so that fall prevention measures are activated appropriately and documented for regulatory compliance.
- **FR-TR-006**: As a triage nurse, I want to set and document isolation precautions (contact, droplet, airborne) for infectious or immunocompromised patients, so that all staff are alerted before entering the patient's room.
- **FR-TR-007**: As a triage nurse, I want to record and reconcile the patient's current home medications during triage, so that prescribers have an accurate medication list before ordering and avoid duplicate therapy or interactions.
- **FR-TR-008**: As a triage nurse or receptionist, I want to verify a patient's Ontario Health Card Number (HCN) and eligibility in real time, so that OHIP-insured services are correctly attributed and ineligible patients are flagged for billing.
- **FR-TR-009**: As a triage nurse, I want to write a structured SBAR (Situation, Background, Assessment, Recommendation) handoff note, so that the receiving physician has a complete, standardised clinical summary without reading the entire chart.
- **FR-TR-010**: As a triage nurse, I want to have the National Early Warning Score 2 (NEWS2) calculated automatically from the vitals I entered, so that deteriorating patients are escalated before they become critically ill.
- **FR-TR-011**: As a triage nurse, I want to verify patient identity using two identifiers (name + DOB, or HCN + name) before recording any clinical data, so that PHI is never entered against the wrong patient record.
- **FR-TR-012**: As a triage nurse, I want to record the patient's pain score on the Numeric Rating Scale (NRS 0–10) and see it alongside previous pain scores, so that clinicians can evaluate pain trajectory and analgesic effectiveness.
- **FR-TR-013**: As a triage nurse or charge nurse, I want to view a real-time patient-status wall board showing all patients in the waiting area with CTAS level, wait time, and assigned state, so that the entire triage team can monitor acuity and workload without refreshing individual patient records.
- **FR-TR-014**: As a triage nurse, I want to record the Glasgow Coma Scale for any patient with altered level of consciousness, so that neurological baseline is documented and changes are tracked.
- **FR-TR-015**: As a triage nurse or receptionist, I want to see which pre-visit intake forms a patient has completed vs outstanding, so that triage can be expedited for patients who have already submitted their history.
- **FR-TR-016**: As a triage nurse, I want to send relevant patient education materials based on the chief complaint, so that patients are informed while waiting and prepared for their consultation.
- **FR-DR-001**: As a physician, I want to write a structured SOAP (Subjective, Objective, Assessment, Plan) clinical note linked to the current encounter, so that the encounter is fully documented, billable, legally defensible, and retrievable by other care providers.
- **FR-DR-002**: As a physician, I want to search for and apply ICD-10-CA diagnosis codes to an encounter, so that clinical and billing documentation is accurate and consistent with Canadian coding standards.
- **FR-DR-003**: As a physician, I want to manage a structured active and historical problem list for each patient, so that any clinician seeing the patient has immediate context of chronic conditions and past significant diagnoses.
- **FR-DR-004**: As a physician, I want to create and send electronic prescriptions directly through PrescribeIT to any pharmacy in Canada, so that patients receive their medications safely, prescriptions cannot be altered, and pharmacists have electronic DUR information.
- **FR-DR-005**: As a physician, I want to order laboratory tests from a comprehensive catalogue (beyond the 4 cardiology procedures), including haematology, chemistry, microbiology, and toxicology, so that patients receive the right diagnostic workup and lab orders flow electronically to the LIMS.
- **FR-DR-006**: As a physician, I want to review laboratory results directly within the encounter view, with abnormal values highlighted and trend graphs available, so that I can make clinical decisions without switching to a separate system.
- **FR-DR-007**: As a physician, I want to view, add, discontinue, and modify the patient's active medication list during the encounter, so that all prescribers and pharmacists have an accurate, current medication list.
- **FR-DR-008**: As a cardiologist, I want to calculate validated clinical risk scores (CHADS-VASc, HAS-BLED, TIMI, HEART, CHA2DS2-VASc) from structured patient data, so that treatment decisions (anticoagulation, discharge, stress test) are evidence-based and documented.
- **FR-DR-009**: As a physician, I want to create an electronic referral to another specialist or facility using a structured FHIR ServiceRequest, so that the receiving provider has complete patient information, and the referral can be tracked through acceptance and scheduling.
- **FR-DR-010**: As a physician, I want to generate a structured discharge summary at the end of an encounter, so that the patient and receiving care providers have a complete record of the visit, diagnosis, treatment, and follow-up plan.
- **FR-DR-011**: As a physician, I want to order radiology studies (CXR, CT, MRI, US) directly from the encounter, so that imaging requests are electronically transmitted to the radiology department with complete clinical context.
- **FR-DR-012**: As a physician, I want to record, view, and update patient consent for procedures, treatment, and data sharing, so that all clinical interventions are documented as having informed consent, satisfying legal and ethical obligations.
- **FR-DR-013**: As a physician, I want to receive evidence-based clinical decision support alerts at key workflow points (prescription, ordering, documentation), so that best-practice guidelines are applied consistently without requiring me to look them up.
- **FR-DR-014**: As a physician, I want to initiate or join a scheduled video consultation with a patient directly from the encounter screen, so that telehealth visits are seamlessly integrated without requiring a separate platform.
- **FR-DR-015**: As a physician, I want to view my daily and weekly schedule, see assigned patients, and mark blocks of unavailability, so that the receptionist can book appointments accurately and I can plan my day.
- **FR-DR-016**: As a physician, I want to flag a case for peer review or request a second opinion from a colleague, so that complex or uncertain cases receive additional clinical oversight.
- **FR-DR-017**: As a physician, I want to create and use reusable clinical note templates for common encounter types, so that documentation is faster, more consistent, and complete across my practice.
- **FR-DR-018**: As a physician, I want to dictate clinical notes by voice and have them transcribed into the SOAP editor, so that documentation time is reduced and I can maintain eye contact with the patient.
- **FR-DR-019**: As a physician, I want to launch third-party SMART on FHIR applications from within the EHR in the context of the current patient encounter, so that specialised tools (drug reference, CDS calculators, genomics apps) integrate seamlessly without re-authentication.
- **FR-DR-020**: As a physician or department head, I want to view outcome and operational analytics for my patient panel (average wait time, readmission rate, top diagnoses, procedure volumes), so that I can identify care gaps and improve clinic performance.
- **FR-LT-001**: As a lab technician, I want to interact with lab orders and results through a functional REST API, so that the LIMS system can receive orders from the EHR and return results electronically.
- **FR-LT-002**: As a lab technician, I want to register specimen collection including accession label printing, tube type, and collection time, so that each specimen is traceable from collection to result.
- **FR-LT-003**: As a lab technician, I want to view a work queue of pending lab orders sorted by priority, STAT first, so that I can process the most urgent work first without manually sorting.
- **FR-LT-004**: As a lab technician or analyst, I want to enter quantitative, qualitative, and coded results for completed lab tests, so that results are available to ordering physicians promptly and accurately.
- **FR-LT-005**: As a lab technician, I want to have automatic escalation workflows trigger when a critical value is detected, so that no critical result goes unacknowledged and patient safety is maintained.
- **FR-LT-006**: As a lab technician, I want to reject specimens that do not meet acceptance criteria and notify the ordering clinician, so that results are not generated from compromised specimens.
- **FR-LT-007**: As a lab technician or LIMS system, I want to send finalised results to the Ontario Laboratories Information System (OLIS), so that any treating provider in Ontario can access the results through the provincial system.
- **FR-LT-008**: As a lab analyst, I want to issue a corrected result when an error is identified in a finalised report, so that the ordering physician receives the accurate result and the original is preserved for audit.
- **FR-LT-009**: As a lab system administrator, I want to define autoverification rules (delta checks, reference range checks, instrument quality checks) so that normal, non-critical results are automatically finalised without manual review, so that turnaround time is reduced and analyst time is focused on abnormal results.
- **FR-LT-010**: As a lab technician (microbiology), I want to record culture results including organism identification and antibiotic sensitivity/resistance profiles, so that physicians can prescribe targeted antibiotic therapy.
- **FR-LT-011**: As a lab manager, I want to view real-time TAT metrics for each test category, so that SLA compliance is monitored and bottlenecks are identified.
- **FR-LT-012**: As a lab technician, I want to enter daily QC results for each analyzer and see Levy-Jennings charts, so that instrument performance is validated before patient results are reported.
- **FR-LT-013**: As a lab system administrator, I want to configure reflex testing rules so that follow-on tests are automatically ordered when primary test results meet defined criteria, so that clinicians receive comprehensive results without manual add-on ordering.
- **FR-LT-014**: As a lab technician, I want to search the lab test catalogue using pCLOCD (Ontario) and LOINC codes, so that test orders from the EHR are correctly mapped and reported back with the correct coding.
- **FR-LT-015**: As a pathologist, I want to receive biopsy and cytology requisitions and issue structured pathology reports, so that surgical and oncology teams receive standardised diagnostic reports.
- **FR-LT-016**: As a lab technician, I want to receive and process cancellation or amendment instructions from the ordering physician in real time, so that unnecessary tests are not performed on collected specimens.
- **FR-LT-017**: As a patient, I want to view my finalised lab results in the patient portal with plain-language explanations, so that I understand my health status without needing to contact my physician.
- **FR-PH-001**: As a pharmacist, I want to access prescriptions, dispense records, and patient medication profiles through a functional REST API, so that PharmacyMS operates as a real clinical system rather than an empty schema.
- **FR-PH-002**: As a pharmacist, I want to receive electronic prescriptions from PrescribeIT-connected physicians automatically, so that manual data entry is eliminated and prescription accuracy is guaranteed.
- **FR-PH-003**: As a pharmacist, I want to receive automated drug interaction, allergy, and duplicate therapy alerts when processing any prescription, so that patient safety is maintained and I can exercise my professional judgment on each alert.
- **FR-PH-004**: As a pharmacist, I want to follow a structured dispensing workflow (verify → fill → label → final check → dispense) with step-by-step documentation, so that each dispensing event is fully traceable and errors are caught before the patient receives the medication.
- **FR-PH-005**: As a pharmacist, I want to adjudicate prescriptions against ODB and private insurance plans electronically, so that the patient's co-pay is calculated correctly and the claim is submitted without manual billing.
- **FR-PH-006**: As a pharmacist, I want to track all controlled substance dispensing with double-count verification and report to Health Canada, so that narcotics and controlled drugs are accounted for in compliance with the Controlled Drugs and Substances Act (CDSA).
- **FR-PH-007**: As a pharmacist, I want to view a complete medication profile for a patient including all active prescriptions, dispensing history, and DHDR data, so that I can provide complete pharmaceutical care and identify therapy gaps.
- **FR-PH-008**: As a pharmacist or pharmacy manager, I want to track drug inventory levels, receive low-stock alerts, and manage drug procurement, so that the pharmacy never runs out of critical medications.
- **FR-PH-009**: As a pharmacist, I want to document compounded preparations including formula, ingredients, and lot number, so that compounded medications are traceable and NAPRA-compliant.
- **FR-PH-010**: As a pharmacist in Ontario, I want to assess, diagnose, and initiate treatment for minor ailments (as permitted under the Pharmacy Act 2021 expansion), so that patients with UTI, allergic rhinitis, or other eligible conditions receive immediate care without waiting for a physician.
- **FR-PH-011**: As a pharmacy owner / billing staff, I want to generate ODB and private insurance claims reports and reconcile payments, so that pharmacy revenue is accurately tracked and discrepancies identified.
- **FR-PH-012**: As a pharmacist, I want to receive Health Canada drug recall alerts and identify affected patients automatically, so that patients who received a recalled medication are notified promptly.
- **FR-PH-013**: As a pharmacist, I want to configure automatic refill reminders for patients with chronic medications, so that patients do not run out of essential medications like blood pressure pills or insulin.
- **FR-PH-014**: As a pharmacist, I want to document patient counselling interactions including topics covered and patient understanding assessment, so that professional documentation demonstrates duty of care and supports continuity of care.
- **FR-PH-015**: As a pharmacy manager, I want to generate compliance reports for the Ontario College of Pharmacists (OCP), so that the pharmacy meets its professional regulatory obligations.
- **FR-PH-016**: As a pharmacy technician or pharmacist, I want to prepare compliance packaging (blister packs/bubble packs) for patients requiring medication management support, so that patients with polypharmacy or cognitive impairment receive their correct medications at the right time.
- **FR-PH-017**: As a pharmacist, I want to send a secure clinical message to the prescribing physician regarding a prescription concern, so that DUR issues, clarifications, and therapy concerns are resolved quickly without using insecure fax.
- **FR-PH-018**: As a pharmacist or technician, I want to verify a patient's identity before releasing a prescription, so that medications are never given to the wrong person.
- **FR-PT-001**: As a pharmacy technician, I want to see a work queue of prescriptions to fill, sorted by waiting time and prescription type, so that I can work efficiently and prioritise urgent fills.
- **FR-PT-002**: As a pharmacy technician, I want to print medication labels from within the dispensing workflow, so that labels are generated automatically and accurately from the prescription data.
- **FR-PT-003**: As a pharmacy technician, I want to submit a completed fill for pharmacist final verification before release, so that no prescription is released to a patient without a pharmacist check.
- **FR-PT-004**: As a pharmacy technician, I want to manage a "ready for pickup" queue so patients are served efficiently, so that wait times at the counter are minimised.
- **FR-PT-005**: As a pharmacy technician, I want to enter prescriptions received by fax or image (OCR-assisted), so that paper-based prescriptions are digitised quickly and accurately.
- **FR-PT-006**: As a pharmacy technician, I want to track medications that are being delivered to patients (curbside pickup, home delivery), so that every prescription can be accounted for and proof of delivery is recorded.
- **FR-PT-007**: As a pharmacy technician, I want to have actions that require a pharmacist (DUR override, controlled substance dispense, final check) automatically flagged and routed, so that I cannot accidentally perform actions outside my scope of practice.
- **FR-PT-008**: As a pharmacy technician (hospital pharmacy), I want to prepare unit-dose medications for inpatient dispensing to specific patient wards, so that nurses can administer the right medication at the right time from the correct unit-dose package.
- **FR-PT-009**: As a pharmacy technician, I want to process auto-refill prescriptions in a batch at the start of the day, so that recurring patients' medications are ready without requiring them to call.
- **FR-PA-001**: As a patient, I want to access a patient-specific portal (not the clinician EHR shell), so that I have an appropriate, privacy-respecting interface designed for my needs, without seeing clinical staff tools.
- **FR-PA-002**: As a patient, I want to book, reschedule, and cancel appointments directly from the patient portal, so that I do not need to call the clinic during business hours.
- **FR-PA-003**: As a patient, I want to receive automated reminders before my appointment, so that I do not forget and the clinic reduces no-shows.
- **FR-PA-004**: As a patient, I want to complete my intake questionnaire online before my appointment, so that my visit is more efficient and the clinical team has my history before I arrive.
- **FR-PA-005**: As a patient, I want to send and receive secure messages with my care team through the patient portal, so that I can ask questions and receive advice without using unsecured email or waiting for a phone appointment.
- **FR-PA-006**: As a patient, I want to view my laboratory results in the patient portal with plain-language explanations, so that I am informed about my health and can prepare questions for my next appointment.
- **FR-PA-007**: As a patient, I want to view my current medication list in the patient portal, so that I can review what I am taking, understand the purpose of each medication, and report discrepancies.
- **FR-PA-008**: As a patient, I want to see a consolidated health summary including active conditions, allergies, immunisations, and recent results, so that I have a complete picture of my health in one place.
- **FR-PA-009**: As a patient, I want to view and manage my privacy consents including who can access my records and what can be shared, so that I have control over my personal health information as guaranteed by PHIPA.
- **FR-PA-010**: As a patient, I want to view my immunisation record in the patient portal, so that I know which vaccines I have received and when.
- **FR-PA-011**: As a patient, I want to join a scheduled telemedicine video call directly from the patient portal, so that I do not need to install separate software or manage separate login credentials.
- **FR-PA-012**: As a patient, I want to view and download clinical documents (discharge summaries, referral letters, lab reports) from my patient portal, so that I have access to my complete health information as required by PHIPA and 21st Century Cures Act.
- **FR-PA-013**: As a patient, I want to update my contact information, communication preferences, and next of kin in the patient portal, so that the clinic always has accurate information for reminders and emergencies.
- **FR-PA-014**: As a Francophone patient, I want to use the patient portal entirely in French, so that I can access my health information in my preferred language as guaranteed by the Official Languages Act.
- **FR-PA-015**: As a patient, I want to view and pay any out-of-pocket fees from my patient portal, so that I can manage my healthcare expenses.
- **FR-PA-016**: As a patient with a chronic condition, I want to share wearable device data (blood pressure cuff, glucometer, smartwatch) with my care team, so that my physician can monitor my condition between visits.
- **FR-PA-017**: As a Ontario patient, I want to log in to the patient portal using my MyHealth Ontario account, so that I do not need to create and manage a separate login.
- **FR-PA-018**: As a patient who is unsure whether to seek care, I want to complete a brief symptom assessment in the patient portal, so that I receive guidance on the urgency of my symptoms.
- **FR-PC-001**: As a system administrator, I want to add PCA/PSW as a distinct clinical role with appropriate permissions, so that patient care assistants can access only the features relevant to their scope of practice.
- **FR-PC-002**: As a patient care assistant, I want to document a patient's ADL status (bathing, dressing, mobility, continence, feeding), so that nursing staff and therapists have current functional status information.
- **FR-PC-003**: As a patient care assistant, I want to record all fluid intake and output throughout the shift, so that nursing staff can monitor fluid balance and detect problems like dehydration or fluid overload.
- **FR-PC-004**: As a patient care assistant, I want to submit a patient transport request (porter, wheelchair, stretcher) from the patient's room, so that patients are moved safely and on time between departments.
- **FR-PC-005**: As a patient care assistant, I want to complete a fall prevention task checklist at the start of each shift for high-risk patients, so that fall prevention measures are documented and consistently applied.
- **FR-PC-006**: As a patient care assistant, I want to enter basic vital signs (temperature, weight) that are then queued for nurse review, so that vital sign collection is efficient while maintaining clinical oversight.
- **FR-PC-007**: As a patient care assistant, I want to log when I have provided educational materials or spoken to a patient about self-care topics, so that the care team is aware of what information the patient has received.
- **FR-PC-008**: As a patient care assistant, I want to follow and document a turning schedule for immobile patients to prevent pressure injuries, so that pressure injury prevention is documented and auditable.
- **FR-RC-001**: As a receptionist, I want to book appointments using a visual day/week/month calendar showing real-time physician availability, so that patients are scheduled correctly without double-booking.
- **FR-RC-002**: As a receptionist, I want to register a new patient including PCR lookup and MRN assignment, so that new patients are correctly identified in the system with an accurate health card and demographics.
- **FR-RC-003**: As a receptionist, I want to verify a patient's OHIP eligibility in real time at check-in, so that insured services are billed correctly and ineligible patients are flagged.
- **FR-RC-004**: As a clinic patient, I want to check in for my appointment via a self-service kiosk, so that I do not have to wait in line at the reception desk.
- **FR-RC-005**: As a receptionist, I want to manage a waitlist for appointments when no immediate slots are available, so that patients are contacted in order when cancellations occur.
- **FR-RC-006**: As a receptionist, I want to mark patients as no-shows and trigger follow-up communications, so that no-show rates are tracked and abandoned slots are filled.
- **FR-RC-007**: As a receptionist, I want to process appointment cancellations with reason codes and slot release, so that the calendar is updated immediately and waitlisted patients are notified.
- **FR-RC-008**: As a receptionist, I want to assign patients to rooms during check-in, so that room occupancy is tracked in real time.
- **FR-RC-009**: As a receptionist, I want to request an interpreter for patients who require language support, so that language is not a barrier to healthcare access.
- **FR-RC-010**: As a clinic administrator, I want to define and manage appointment types (new, follow-up, procedure, urgent, telemedicine) with slot durations and booking rules, so that scheduling is consistent and appropriate slots are allocated.
- **FR-RC-011**: As a receptionist, I want to receive incoming referrals (electronic or fax), review them, and schedule the appointment, so that referred patients are booked promptly and within wait-time targets.
- **FR-RC-012**: As a receptionist or clinic manager, I want to run an end-of-day reconciliation showing all visits opened, closed, no-showed, and pending, so that the clinic has a complete daily summary for billing and operational review.
- **FR-RC-013**: As a receptionist, I want to quickly send a delay notification to all waiting patients when a physician is running late, so that patients in the waiting room and those not yet arrived are informed.
- **FR-RC-014**: As a receptionist or clinic manager, I want to identify patients overdue for preventive care and send recall notices, so that the patient panel maintains preventive care compliance.
- **FR-RC-015**: As a receptionist, I want to prompt patients to confirm their demographics at each check-in and update as needed, so that the patient record is current and OHIP billing information is accurate.
- **FR-RC-016**: As a patient in the waiting room, I want to see an estimated wait time for my appointment on the waiting room display, so that I can manage my time and know when to expect to be called.
- **FR-RC-017**: As a receptionist at a multi-site clinic, I want to schedule appointments across multiple locations and providers in a single interface, so that I can direct patients to the best available option without switching between systems.
- **FR-RC-018**: As a receptionist, I want to book telemedicine appointments with automated join-link generation, so that patients receive all the information they need for a virtual visit.
- **FR-AD-001**: As a system administrator, I want to have every access to patient health information logged immutably with actor, action, resource, and timestamp, so that the organisation can demonstrate PHIPA §12 compliance and investigate privacy breaches.
- **FR-AD-002**: As a system administrator, I want to ensure that new registrations default to a PENDING role requiring admin approval, so that unauthorized users cannot self-assign DOCTOR or ADMIN roles and access clinical data.
- **FR-AD-003**: As a system administrator, I want to ensure that all API routes enforce role-based access control at the server level, so that frontend restrictions cannot be bypassed by direct API calls.
- **FR-AD-004**: As a system administrator, I want to create, read, update, deactivate, and manage all user accounts, so that staff access is controlled accurately from day one to offboarding.
- **FR-AD-005**: As a system administrator, I want to view the status of all integration endpoints (OLIS, PrescribeIT, FHIR, DHDR, PCR) in real time, so that I can identify and resolve integration failures before they affect patient care.
- **FR-AD-006**: As a system administrator, I want to configure clinic settings including working hours, appointment types, room list, and staff schedule templates, so that the system reflects the real-world operations of each clinic.
- **FR-AD-007**: As a system administrator, I want to view and terminate active user sessions, so that I can respond to security incidents by immediately revoking access.
- **FR-AD-008**: As a system administrator, I want to view real-time system health metrics (CPU, memory, database connections, error rates, response times), so that performance issues are identified before they affect users.
- **FR-AD-009**: As a system administrator or clinical informatics lead, I want to manage the code systems, value sets, and concept maps used in the system, so that clinical coding is consistent with current Canadian and international standards.
- **FR-AD-010**: As a clinic manager, I want to export de-identified operational data for quality improvement and regulatory reporting, so that the clinic meets reporting obligations to OHQC, accreditation bodies, and funders.
- **FR-AD-011**: As a system administrator, I want to have automated daily backups and a tested restore procedure, so that in the event of data loss or disaster, the system can be recovered within the defined RTO/RPO.
- **FR-AD-012**: As a system administrator, I want to ensure all PHI stored in the database is encrypted at rest, so that even if a physical storage device is compromised, patient data cannot be read.
- **FR-AD-013**: As a system administrator, I want to enforce API rate limits to protect the system from abuse and denial-of-service attacks, so that legitimate clinical users are not impacted by excessive automated requests.
- **FR-AD-014**: As a system administrator, I want to broadcast system announcements (planned maintenance, downtime, policy changes) to all logged-in users, so that staff are informed without using external email.
- **FR-AD-015**: As a system administrator and developer, I want to ensure that all clinical UI pages have error boundaries that degrade gracefully, so that a crash in one component does not leave a clinician unable to access any patient information.
- **FR-BI-001**: As a billing specialist, I want to see a queue of completed encounters requiring coding and claim submission, so that claims are processed promptly and revenue is not lost.
- **FR-BI-002**: As a billing specialist, I want to code encounters with ICD-10-CA diagnosis codes for OHIP claim submission, so that claims are accepted by the Ministry of Health.
- **FR-BI-003**: As a billing specialist, I want to select applicable OHIP fee codes for services rendered, so that the claim reflects all billable services and the correct amount is submitted.
- **FR-BI-004**: As a billing specialist, I want to submit OHIP claims electronically via Ministry Connections Electronic Data Transfer (MC EDT), so that claims are submitted within the billing period and payment is received.
- **FR-BI-005**: As a billing specialist, I want to manage rejected OHIP and insurance claims and resubmit after correction, so that revenue that was initially rejected is recovered.
- **FR-BI-006**: As a billing specialist, I want to generate invoices for patients whose services are not covered by OHIP, so that the clinic receives payment for uninsured services.
- **FR-BI-007**: As a clinic manager, I want to view revenue cycle KPIs including gross charges, collections, rejection rate, and accounts receivable aging, so that the financial health of the clinic is visible and trends are tracked.
- **FR-BI-008**: As a billing specialist, I want to receive alerts for claims approaching the OHIP 6-month submission deadline, so that no billable encounter is lost due to missed submission.
- **FR-BI-009**: As a billing specialist, I want to generate a superbill (billing summary) directly from the encounter documentation, so that billing accurately reflects the services documented by the physician.
- **FR-BI-010**: As a patient, I want to receive a clear itemised billing statement for any services not fully covered by OHIP, so that I understand what I am paying for and can resolve discrepancies.
- **FR-TK-001**: As a cardiac technician, I want to enter a structured ECG result including rhythm, rate, intervals, and interpretation, so that cardiologists can review a complete, standardised report without receiving a raw paper trace.
- **FR-TK-002**: As a cardiac technician (sonographer), I want to enter a structured echocardiogram report with EF, wall motion, valve assessments, and dimensions, so that cardiologists receive standardised echo data without free-text ambiguity.
- **FR-TK-003**: As a cardiac technician, I want to document a stress test including protocol used, stages completed, symptoms, ECG changes, and haemodynamic response, so that the cardiologist has complete data to interpret the test.
- **FR-TK-004**: As a cardiac technician, I want to assign a Holter monitor device to a patient, set the wear duration, and process the returned monitor, so that Holter data is linked to the correct patient and processed for arrhythmia analysis.
- **FR-TK-005**: As a cardiac technician or cardiologist, I want to view DICOM imaging (Echo cine loops, stress test images) directly within the EHR, so that clinical review does not require a separate PACS workstation.
- **FR-TK-006**: As a cardiac technician, I want to flag critical findings and have an automated escalation workflow fire, so that no critical cardiac finding goes unacknowledged by the responsible cardiologist.
- **FR-TK-007**: As a cardiac technician or cardiologist, I want to compare current procedure results against previous studies from the same patient, so that progression or regression of disease is immediately visible.
- **FR-TK-008**: As a cardiac technician or department manager, I want to view the workload distribution across technicians and procedure queues, so that work is balanced and no queue is overloaded.
- **FR-TK-009**: As a cardiac technician, I want to ensure that all completed procedure results are stored as FHIR DiagnosticReport resources in the FHIR server, so that results are accessible via standard FHIR queries by any integrated system.
- **FR-TK-010**: As a cardiac technician, I want to view equipment status (available / in use / maintenance) for each procedure room, so that I can select the correct room and avoid scheduling conflicts.
- **FR-CC-001**: As a clinical staff member, I want to receive real-time push notifications for critical events (critical lab values, patient arrivals, new orders) without polling, so that urgent clinical information reaches me immediately rather than after a 30-second polling delay.
- **FR-CC-002**: As a system architect, I want to implement Temporal.io for long-running clinical workflows (lab result routing, prescription lifecycle, appointment reminder chain), so that complex multi-step processes are reliable, retryable, and observable without writing custom state machines.
- **FR-CC-003**: As a receptionist or clinician, I want to create and edit patient demographic records that are backed by FHIR Patient resources, so that all applications share a consistent, authoritative patient identity.
- **FR-CC-004**: As a system architect, I want to define and implement standard FHIR API contracts between EHR, LIMS, and PharmacyMS, so that data flows seamlessly across applications without proprietary point-to-point integrations.
- **FR-CC-005**: As a FHIR server administrator, I want to implement a standards-compliant SMART on FHIR authorization server, so that third-party SMART apps can launch in context without requiring separate login credentials.
- **FR-CC-006**: As a receptionist or triage nurse, I want to look up and verify patient identity against the Ontario Provincial Client Registry, so that patient records are linked to the provincial master patient index.
- **FR-CC-007**: As a French-speaking healthcare worker or patient, I want to use all features of the system in Canadian French, so that the Official Languages Act obligation is met for any federal or Ontario francophone context.
- **FR-CC-008**: As a clinical staff member in an area with intermittent connectivity, I want to continue viewing patient information and recording basic data during network outages, so that patient care is not disrupted by connectivity issues.
- **FR-CC-009**: As a nurse or PCA using a tablet, I want to use the clinical application comfortably on a 768px tablet screen, so that bedside care documentation is practical on mobile devices.
- **FR-CC-010**: As a FHIR server administrator, I want to validate all FHIR resources against the appropriate Canadian (CA Baseline, ON Health IG) and US Core profiles before storage, so that stored resources are interoperable with provincial and national health systems.
- **FR-CC-011**: As a clinical user, I want to receive clear, actionable error messages when something goes wrong, so that I know what happened and what I can do to resolve it.
- **FR-CC-012**: As a developer and QA engineer, I want to have a comprehensive Playwright e2e test suite covering critical clinical workflows, so that regressions in patient safety-critical paths are caught before production deployment.
- **FR-CC-013**: As a developer, I want to maintain minimum unit test coverage on all clinical logic functions, so that business rules (CTAS classification, NEWS2 calculation, DUR engine, billing rules) are verified automatically.
- **FR-CC-014**: As a system administrator, I want to know that the platform can handle peak clinical load without performance degradation, so that the system does not slow down during busy clinic hours.
- **FR-CC-015**: As a system architect, I want to define and implement a unified audit logging strategy using FHIR AuditEvent across all four applications, so that PHIPA compliance is met and a single audit query can trace any PHI access across the entire platform.
- **FR-CC-016**: As a system administrator or data analyst, I want to export large datasets of de-identified FHIR resources using the FHIR $export operation, so that population health analytics and research can be conducted on the platform data.

### NonFunctional Requirements

- NFR1: All PHI access must be logged (actor, timestamp, entity) per PHIPA §12; audit log must be tamper-evident and DB-backed.
- NFR2: WCAG 2.2 Level AA accessibility across all clinician and patient-facing UI (AODA IASR / Section 508 / ADA).
- NFR3: TLS 1.3 enforced in transit for all production traffic; PHI encrypted at rest.
- NFR4: OAuth 2.0 / SMART on FHIR used for all cross-application and third-party authorization.
- NFR5: Real-time critical alerts (panic values, CTAS-1 escalation) must reach staff dashboards within 2 seconds.
- NFR6: API rate limiting (429 + Retry-After) on all public and inter-service endpoints; brute-force protection on auth endpoints.
- NFR7: Bilingual EN/FR support for pan-Canadian and Ontario-facing patient portal screens.
- NFR8: FHIR R4 resource validation against Ontario/CA Baseline and US Core profiles where applicable.
- NFR9: All destructive or state-changing clinical actions must emit a domain event for audit and downstream integration.
- NFR10: System must degrade gracefully — SSE fallback when WebSocket is blocked; offline-tolerant PWA behaviour for patient portal.

### Additional Requirements

- GAP-001..005 (P0, must-fix before clinical use): self-registration role exploit, DB-backed PHIPA audit log, per-record PHI access logging, TLS enforcement in production, API-level RBAC enforcement (see overall-lld.md §15).
- GAP-010..021 (P1, core clinical workflows): LIMS + PharmacyMS REST APIs, patient demographics CRUD, WebSocket/SSE via Redis pub/sub, panic value notification, SOAP note structure, ICD-10-CA coding, CTAS triage, cardiology calculators, DiagnosticReport display, problem list, allergy recording.
- GAP-030..035 (P2, patient experience): patient portal shell, self-booking, lab results access, secure messaging, pre-visit questionnaires, EN/FR bilingual support.
- GAP-040..045 (P3, provincial interoperability): PrescribeIT adapter, OLIS outbound, PCR patient matching, OHIP verification, DHDR read integration, OHIP billing.
- GAP-050..054 (P4, advanced): CDS Hooks engine, SMART on FHIR launch, DICOM viewer, Temporal workflow engine, AI-assisted documentation.
- Port map / service boundaries: EHR :3000 (Next.js/Prisma), FHIR Server :8081 (Go), PharmacyMS :8082 (Go), LIMS :8083 (Go), PostgreSQL :5432, Redis :6379 (not yet deployed).

### UX Design Requirements

_No platform-wide UX Design Specification exists for this PRD. A UX doc exists under the separate `ehr-core-clinical` initiative but is out of scope for this epic set._

### FR Coverage Map

| FR ID | Epic | Story |
|---|---|---|
| FR-TR-001 | Epic 1 | Story 1.1 |
| FR-TR-002 | Epic 1 | Story 1.2 |
| FR-TR-003 | Epic 1 | Story 1.3 |
| FR-TR-004 | Epic 1 | Story 1.4 |
| FR-TR-005 | Epic 1 | Story 1.5 |
| FR-TR-006 | Epic 1 | Story 1.6 |
| FR-TR-007 | Epic 1 | Story 1.7 |
| FR-TR-008 | Epic 1 | Story 1.8 |
| FR-TR-009 | Epic 1 | Story 1.9 |
| FR-TR-010 | Epic 1 | Story 1.10 |
| FR-TR-011 | Epic 1 | Story 1.11 |
| FR-TR-012 | Epic 1 | Story 1.12 |
| FR-TR-013 | Epic 1 | Story 1.13 |
| FR-TR-014 | Epic 1 | Story 1.14 |
| FR-TR-015 | Epic 1 | Story 1.15 |
| FR-TR-016 | Epic 1 | Story 1.16 |
| FR-DR-001 | Epic 2 | Story 2.1 |
| FR-DR-002 | Epic 2 | Story 2.2 |
| FR-DR-003 | Epic 2 | Story 2.3 |
| FR-DR-004 | Epic 2 | Story 2.4 |
| FR-DR-005 | Epic 2 | Story 2.5 |
| FR-DR-006 | Epic 2 | Story 2.6 |
| FR-DR-007 | Epic 2 | Story 2.7 |
| FR-DR-008 | Epic 2 | Story 2.8 |
| FR-DR-009 | Epic 2 | Story 2.9 |
| FR-DR-010 | Epic 2 | Story 2.10 |
| FR-DR-011 | Epic 2 | Story 2.11 |
| FR-DR-012 | Epic 2 | Story 2.12 |
| FR-DR-013 | Epic 2 | Story 2.13 |
| FR-DR-014 | Epic 2 | Story 2.14 |
| FR-DR-015 | Epic 2 | Story 2.15 |
| FR-DR-016 | Epic 2 | Story 2.16 |
| FR-DR-017 | Epic 2 | Story 2.17 |
| FR-DR-018 | Epic 2 | Story 2.18 |
| FR-DR-019 | Epic 2 | Story 2.19 |
| FR-DR-020 | Epic 2 | Story 2.20 |
| FR-LT-001 | Epic 3 | Story 3.1 |
| FR-LT-002 | Epic 3 | Story 3.2 |
| FR-LT-003 | Epic 3 | Story 3.3 |
| FR-LT-004 | Epic 3 | Story 3.4 |
| FR-LT-005 | Epic 3 | Story 3.5 |
| FR-LT-006 | Epic 3 | Story 3.6 |
| FR-LT-007 | Epic 3 | Story 3.7 |
| FR-LT-008 | Epic 3 | Story 3.8 |
| FR-LT-009 | Epic 3 | Story 3.9 |
| FR-LT-010 | Epic 3 | Story 3.10 |
| FR-LT-011 | Epic 3 | Story 3.11 |
| FR-LT-012 | Epic 3 | Story 3.12 |
| FR-LT-013 | Epic 3 | Story 3.13 |
| FR-LT-014 | Epic 3 | Story 3.14 |
| FR-LT-015 | Epic 3 | Story 3.15 |
| FR-LT-016 | Epic 3 | Story 3.16 |
| FR-LT-017 | Epic 3 | Story 3.17 |
| FR-PH-001 | Epic 4 | Story 4.1 |
| FR-PH-002 | Epic 4 | Story 4.2 |
| FR-PH-003 | Epic 4 | Story 4.3 |
| FR-PH-004 | Epic 4 | Story 4.4 |
| FR-PH-005 | Epic 4 | Story 4.5 |
| FR-PH-006 | Epic 4 | Story 4.6 |
| FR-PH-007 | Epic 4 | Story 4.7 |
| FR-PH-008 | Epic 4 | Story 4.8 |
| FR-PH-009 | Epic 4 | Story 4.9 |
| FR-PH-010 | Epic 4 | Story 4.10 |
| FR-PH-011 | Epic 4 | Story 4.11 |
| FR-PH-012 | Epic 4 | Story 4.12 |
| FR-PH-013 | Epic 4 | Story 4.13 |
| FR-PH-014 | Epic 4 | Story 4.14 |
| FR-PH-015 | Epic 4 | Story 4.15 |
| FR-PH-016 | Epic 4 | Story 4.16 |
| FR-PH-017 | Epic 4 | Story 4.17 |
| FR-PH-018 | Epic 4 | Story 4.18 |
| FR-PT-001 | Epic 5 | Story 5.1 |
| FR-PT-002 | Epic 5 | Story 5.2 |
| FR-PT-003 | Epic 5 | Story 5.3 |
| FR-PT-004 | Epic 5 | Story 5.4 |
| FR-PT-005 | Epic 5 | Story 5.5 |
| FR-PT-006 | Epic 5 | Story 5.6 |
| FR-PT-007 | Epic 5 | Story 5.7 |
| FR-PT-008 | Epic 5 | Story 5.8 |
| FR-PT-009 | Epic 5 | Story 5.9 |
| FR-PA-001 | Epic 6 | Story 6.1 |
| FR-PA-002 | Epic 6 | Story 6.2 |
| FR-PA-003 | Epic 6 | Story 6.3 |
| FR-PA-004 | Epic 6 | Story 6.4 |
| FR-PA-005 | Epic 6 | Story 6.5 |
| FR-PA-006 | Epic 6 | Story 6.6 |
| FR-PA-007 | Epic 6 | Story 6.7 |
| FR-PA-008 | Epic 6 | Story 6.8 |
| FR-PA-009 | Epic 6 | Story 6.9 |
| FR-PA-010 | Epic 6 | Story 6.10 |
| FR-PA-011 | Epic 6 | Story 6.11 |
| FR-PA-012 | Epic 6 | Story 6.12 |
| FR-PA-013 | Epic 6 | Story 6.13 |
| FR-PA-014 | Epic 6 | Story 6.14 |
| FR-PA-015 | Epic 6 | Story 6.15 |
| FR-PA-016 | Epic 6 | Story 6.16 |
| FR-PA-017 | Epic 6 | Story 6.17 |
| FR-PA-018 | Epic 6 | Story 6.18 |
| FR-PC-001 | Epic 7 | Story 7.1 |
| FR-PC-002 | Epic 7 | Story 7.2 |
| FR-PC-003 | Epic 7 | Story 7.3 |
| FR-PC-004 | Epic 7 | Story 7.4 |
| FR-PC-005 | Epic 7 | Story 7.5 |
| FR-PC-006 | Epic 7 | Story 7.6 |
| FR-PC-007 | Epic 7 | Story 7.7 |
| FR-PC-008 | Epic 7 | Story 7.8 |
| FR-RC-001 | Epic 8 | Story 8.1 |
| FR-RC-002 | Epic 8 | Story 8.2 |
| FR-RC-003 | Epic 8 | Story 8.3 |
| FR-RC-004 | Epic 8 | Story 8.4 |
| FR-RC-005 | Epic 8 | Story 8.5 |
| FR-RC-006 | Epic 8 | Story 8.6 |
| FR-RC-007 | Epic 8 | Story 8.7 |
| FR-RC-008 | Epic 8 | Story 8.8 |
| FR-RC-009 | Epic 8 | Story 8.9 |
| FR-RC-010 | Epic 8 | Story 8.10 |
| FR-RC-011 | Epic 8 | Story 8.11 |
| FR-RC-012 | Epic 8 | Story 8.12 |
| FR-RC-013 | Epic 8 | Story 8.13 |
| FR-RC-014 | Epic 8 | Story 8.14 |
| FR-RC-015 | Epic 8 | Story 8.15 |
| FR-RC-016 | Epic 8 | Story 8.16 |
| FR-RC-017 | Epic 8 | Story 8.17 |
| FR-RC-018 | Epic 8 | Story 8.18 |
| FR-AD-001 | Epic 9 | Story 9.1 |
| FR-AD-002 | Epic 9 | Story 9.2 |
| FR-AD-003 | Epic 9 | Story 9.3 |
| FR-AD-004 | Epic 9 | Story 9.4 |
| FR-AD-005 | Epic 9 | Story 9.5 |
| FR-AD-006 | Epic 9 | Story 9.6 |
| FR-AD-007 | Epic 9 | Story 9.7 |
| FR-AD-008 | Epic 9 | Story 9.8 |
| FR-AD-009 | Epic 9 | Story 9.9 |
| FR-AD-010 | Epic 9 | Story 9.10 |
| FR-AD-011 | Epic 9 | Story 9.11 |
| FR-AD-012 | Epic 9 | Story 9.12 |
| FR-AD-013 | Epic 9 | Story 9.13 |
| FR-AD-014 | Epic 9 | Story 9.14 |
| FR-AD-015 | Epic 9 | Story 9.15 |
| FR-BI-001 | Epic 10 | Story 10.1 |
| FR-BI-002 | Epic 10 | Story 10.2 |
| FR-BI-003 | Epic 10 | Story 10.3 |
| FR-BI-004 | Epic 10 | Story 10.4 |
| FR-BI-005 | Epic 10 | Story 10.5 |
| FR-BI-006 | Epic 10 | Story 10.6 |
| FR-BI-007 | Epic 10 | Story 10.7 |
| FR-BI-008 | Epic 10 | Story 10.8 |
| FR-BI-009 | Epic 10 | Story 10.9 |
| FR-BI-010 | Epic 10 | Story 10.10 |
| FR-TK-001 | Epic 11 | Story 11.1 |
| FR-TK-002 | Epic 11 | Story 11.2 |
| FR-TK-003 | Epic 11 | Story 11.3 |
| FR-TK-004 | Epic 11 | Story 11.4 |
| FR-TK-005 | Epic 11 | Story 11.5 |
| FR-TK-006 | Epic 11 | Story 11.6 |
| FR-TK-007 | Epic 11 | Story 11.7 |
| FR-TK-008 | Epic 11 | Story 11.8 |
| FR-TK-009 | Epic 11 | Story 11.9 |
| FR-TK-010 | Epic 11 | Story 11.10 |
| FR-CC-001 | Epic 12 | Story 12.1 |
| FR-CC-002 | Epic 12 | Story 12.2 |
| FR-CC-003 | Epic 12 | Story 12.3 |
| FR-CC-004 | Epic 12 | Story 12.4 |
| FR-CC-005 | Epic 12 | Story 12.5 |
| FR-CC-006 | Epic 12 | Story 12.6 |
| FR-CC-007 | Epic 12 | Story 12.7 |
| FR-CC-008 | Epic 12 | Story 12.8 |
| FR-CC-009 | Epic 12 | Story 12.9 |
| FR-CC-010 | Epic 12 | Story 12.10 |
| FR-CC-011 | Epic 12 | Story 12.11 |
| FR-CC-012 | Epic 12 | Story 12.12 |
| FR-CC-013 | Epic 12 | Story 12.13 |
| FR-CC-014 | Epic 12 | Story 12.14 |
| FR-CC-015 | Epic 12 | Story 12.15 |
| FR-CC-016 | Epic 12 | Story 12.16 |

## Epic List

- **Epic 1: Triage Nurse — Assessment & Intake Workflows** — 16 stories
- **Epic 2: Physician / Cardiologist — Clinical Documentation & Orders** — 20 stories
- **Epic 3: Lab Technician — LIMS Order & Result Workflows** — 17 stories
- **Epic 4: Pharmacist — Dispensing, DUR & Compliance** — 18 stories
- **Epic 5: Pharmacy Technician — Fulfillment Workflows** — 9 stories
- **Epic 6: Patient — Portal & Self-Service** — 18 stories
- **Epic 7: Patient Care Assistant — Bedside Support** — 8 stories
- **Epic 8: Receptionist / Front Desk — Scheduling & Check-In** — 18 stories
- **Epic 9: System Administrator — Security, Config & Compliance** — 15 stories
- **Epic 10: Billing Specialist — Coding & Revenue Cycle** — 10 stories
- **Epic 11: Cardiac Technician — Procedure & Diagnostic Workflows** — 10 stories
- **Epic 12: Cross-Cutting — Platform, Integration & Quality** — 16 stories

## Epic 1: Triage Nurse — Assessment & Intake Workflows

Enable triage nurses to safely and efficiently assess, classify, and hand off arriving patients with full clinical documentation (CTAS, vitals, allergies, fall risk, isolation, medication reconciliation) integrated into the FHIR record.

### Story 1.1: TR-001 — CTAS Triage Classification
...
### Story 1.2: TR-002 — Structured Triage Form with Chief Complaint
...
### Story 1.3: TR-003 — Full Vital Signs Capture (Extended Set)
...
### Story 1.4: TR-004 — Allergy Recording and Verification
...
### Story 1.5: TR-005 — Fall Risk Assessment (Morse Fall Scale)
...
### Story 1.6: TR-006 — Isolation Precautions Entry
...
### Story 1.7: TR-007 — Medication Reconciliation at Triage
...
### Story 1.8: TR-008 — OHIP / Health Card Verification at Triage
...
### Story 1.9: TR-009 — Handoff Notes (SBAR Format)
...
### Story 1.10: TR-010 — Early Warning Score Auto-Calculation (NEWS2)
...
### Story 1.11: TR-011 — Patient Identification Verification
...
### Story 1.12: TR-012 — Pain Score Capture and Trending
...
### Story 1.13: TR-013 — Triage Queue Wall Board
...
### Story 1.14: TR-014 — Glasgow Coma Scale (GCS) Entry
...
### Story 1.15: TR-015 — Intake Form Tracking (PRE_VISIT_FORMS)
...
### Story 1.16: TR-016 — Patient Education Materials Dispatch
...
## Epic 2: Physician / Cardiologist — Clinical Documentation & Orders

Give physicians a complete clinical workflow — structured notes, coded diagnoses, orders, prescriptions, referrals, and decision support — so encounters are documented safely, efficiently, and to a coded, interoperable standard.

### Story 2.1: DR-001 — Structured SOAP Clinical Note
...
### Story 2.2: DR-002 — ICD-10-CA Diagnosis Coding
...
### Story 2.3: DR-003 — Problem List Management
...
### Story 2.4: DR-004 — Electronic Prescription (e-Rx) with PrescribeIT
...
### Story 2.5: DR-005 — Lab Order Entry (Full Panel)
...
### Story 2.6: DR-006 — Lab Result Review (In-Context)
...
### Story 2.7: DR-007 — Medication List Management (Active Meds)
...
### Story 2.8: DR-008 — Cardiology Risk Score Calculators
...
### Story 2.9: DR-009 — Referral Generation (Outbound eReferral)
...
### Story 2.10: DR-010 — Discharge Summary Generation
...
### Story 2.11: DR-011 — Imaging / Radiology Order
...
### Story 2.12: DR-012 — Consent Management
...
### Story 2.13: DR-013 — CDS Hooks Integration
...
### Story 2.14: DR-014 — Telemedicine / Video Consult
...
### Story 2.15: DR-015 — Physician Schedule / Availability
...
### Story 2.16: DR-016 — Second Opinion / Peer Review Flag
...
### Story 2.17: DR-017 — Clinical Note Template Library
...
### Story 2.18: DR-018 — Voice Dictation / Transcription for Notes
...
### Story 2.19: DR-019 — SMART on FHIR App Launch
...
### Story 2.20: DR-020 — Outpatient Clinical Analytics Dashboard
...
## Epic 3: Lab Technician — LIMS Order & Result Workflows

Provide lab technicians and analysts a functioning LIMS: order intake, specimen handling, result entry, critical value escalation, QC, and OLIS-ready reporting.

### Story 3.1: LT-001 — LIMS REST API — Core Endpoints
...
### Story 3.2: LT-002 — Specimen Collection Workflow
...
### Story 3.3: LT-003 — Lab Order Work Queue
...
### Story 3.4: LT-004 — Lab Result Entry
...
### Story 3.5: LT-005 — Panic Value Escalation
...
### Story 3.6: LT-006 — Specimen Rejection Workflow
...
### Story 3.7: LT-007 — OLIS Outbound Result Submission
...
### Story 3.8: LT-008 — Amended / Corrected Result
...
### Story 3.9: LT-009 — Autoverification Rules
...
### Story 3.10: LT-010 — Microbiology Culture & Sensitivity
...
### Story 3.11: LT-011 — TAT (Turnaround Time) Monitoring
...
### Story 3.12: LT-012 — Quality Control (QC) Management
...
### Story 3.13: LT-013 — Reflex Testing Rules
...
### Story 3.14: LT-014 — pCLOCD Test Catalogue
...
### Story 3.15: LT-015 — Pathology / Anatomic Pathology Module
...
### Story 3.16: LT-016 — Order Amendment / Cancellation from EHR
...
### Story 3.17: LT-017 — Patient-Facing Lab Result View
...
## Epic 4: Pharmacist — Dispensing, DUR & Compliance

Equip pharmacists with a safe, compliant dispensing workflow — DUR screening, e-prescription intake, insurance adjudication, controlled substance tracking, and regulatory reporting.

### Story 4.1: PH-001 — PharmacyMS Core REST API
...
### Story 4.2: PH-002 — Electronic Prescription Intake (PrescribeIT)
...
### Story 4.3: PH-003 — Drug Utilisation Review (DUR)
...
### Story 4.4: PH-004 — Dispensing Workflow
...
### Story 4.5: PH-005 — Ontario Drug Benefit (ODB) and Insurance Adjudication
...
### Story 4.6: PH-006 — Controlled Substance Tracking
...
### Story 4.7: PH-007 — Patient Medication Profile
...
### Story 4.8: PH-008 — Medication Inventory and Stock Management
...
### Story 4.9: PH-009 — Compounding Module
...
### Story 4.10: PH-010 — Pharmacist-Initiated Therapy (Minor Ailment)
...
### Story 4.11: PH-011 — Pharmacy Billing and Adjudication Reporting
...
### Story 4.12: PH-012 — Drug Recall Management
...
### Story 4.13: PH-013 — Medication Refill Reminder
...
### Story 4.14: PH-014 — Pharmacist Counselling Record
...
### Story 4.15: PH-015 — NAPRA / OCP Compliance Reporting
...
### Story 4.16: PH-016 — Blister Pack / Compliance Packaging
...
### Story 4.17: PH-017 — Pharmacist-to-Prescriber Messaging
...
### Story 4.18: PH-018 — Patient Identity Verification at Pickup
...
## Epic 5: Pharmacy Technician — Fulfillment Workflows

Support pharmacy technicians in filling, labelling, verifying, and delivering prescriptions within their scope of practice.

### Story 5.1: PT-001 — Technician Work Queue
...
### Story 5.2: PT-002 — Label Printing
...
### Story 5.3: PT-003 — Pharmacist Final Verification Step
...
### Story 5.4: PT-004 — Ready for Pickup Queue
...
### Story 5.5: PT-005 — Fax / Image Prescription Entry
...
### Story 5.6: PT-006 — Patient Delivery / Curbside Tracking
...
### Story 5.7: PT-007 — Scope of Practice Enforcement
...
### Story 5.8: PT-008 — Medication Unit Dose Packaging (Hospital)
...
### Story 5.9: PT-009 — Auto-Refill Processing (Technician-Initiated)
...
## Epic 6: Patient — Portal & Self-Service

Deliver a secure, bilingual, accessible patient portal for booking, results, messaging, consent, and health summary access.

### Story 6.1: PA-001 — Patient Portal — Dedicated Shell
...
### Story 6.2: PA-002 — Appointment Self-Booking
...
### Story 6.3: PA-003 — Appointment Reminders
...
### Story 6.4: PA-004 — Pre-Visit Questionnaire (Online Intake)
...
### Story 6.5: PA-005 — Secure Messaging to Care Team
...
### Story 6.6: PA-006 — Lab Results Patient Access
...
### Story 6.7: PA-007 — Medication List View (Patient-Facing)
...
### Story 6.8: PA-008 — Health Summary View (Patient)
...
### Story 6.9: PA-009 — PHIPA Privacy Consent Management
...
### Story 6.10: PA-010 — Immunisation History View
...
### Story 6.11: PA-011 — Virtual Visit Join
...
### Story 6.12: PA-012 — Document Access (Reports, Summaries, Letters)
...
### Story 6.13: PA-013 — Patient Profile and Demographics Self-Update
...
### Story 6.14: PA-014 — French Language Support (Bilingual Portal)
...
### Story 6.15: PA-015 — Patient Bill Viewing
...
### Story 6.16: PA-016 — Wearable / Remote Monitoring Data Upload
...
### Story 6.17: PA-017 — MyHealth Ontario / Provincial SSO
...
### Story 6.18: PA-018 — Patient Symptom Checker (Pre-Triage)
...
## Epic 7: Patient Care Assistant — Bedside Support

Enable PCAs to document ADLs, I&O, transport, and fall-prevention tasks within their defined scope of practice.

### Story 7.1: PC-001 — PCA Role in Role Model
...
### Story 7.2: PC-002 — ADL (Activities of Daily Living) Documentation
...
### Story 7.3: PC-003 — Intake and Output (I&O) Tracking
...
### Story 7.4: PC-004 — Patient Transport Request
...
### Story 7.5: PC-005 — Fall Prevention Task Checklist
...
### Story 7.6: PC-006 — Vital Signs Assist (Limited Entry)
...
### Story 7.7: PC-007 — Patient Education Delivery Logging
...
### Story 7.8: PC-008 — Turning / Repositioning Schedule
...
## Epic 8: Receptionist / Front Desk — Scheduling & Check-In

Provide front-desk staff full scheduling, check-in, eligibility verification, and recall/no-show management capability.

### Story 8.1: RC-001 — Appointment Scheduling Calendar
...
### Story 8.2: RC-002 — New Patient Registration
...
### Story 8.3: RC-003 — OHIP Eligibility Verification at Check-In
...
### Story 8.4: RC-004 — Check-In Kiosk Mode
...
### Story 8.5: RC-005 — Patient Waitlist Management
...
### Story 8.6: RC-006 — No-Show Management
...
### Story 8.7: RC-007 — Cancellation Workflow
...
### Story 8.8: RC-008 — Room Assignment
...
### Story 8.9: RC-009 — Interpreter Request at Booking
...
### Story 8.10: RC-010 — Appointment Type Catalogue Management
...
### Story 8.11: RC-011 — Referral Intake and Scheduling
...
### Story 8.12: RC-012 — End-of-Day Reconciliation
...
### Story 8.13: RC-013 — Run-Time Delay Patient Notification
...
### Story 8.14: RC-014 — Recall / Preventive Care Recall Management
...
### Story 8.15: RC-015 — Patient Demographics Update at Check-In
...
### Story 8.16: RC-016 — Wait Time Display for Patients
...
### Story 8.17: RC-017 — Multi-Location / Multi-Provider Scheduling
...
### Story 8.18: RC-018 — Appointment Type: Telemedicine Booking
...
## Epic 9: System Administrator — Security, Config & Compliance

Close safety-critical security and compliance gaps — audit logging, RBAC enforcement, encryption, session management, and system configuration.

### Story 9.1: AD-001 — PHIPA-Compliant PHI Audit Log
...
### Story 9.2: AD-002 — Fix Self-Registration Role Exploit
...
### Story 9.3: AD-003 — API-Level RBAC Enforcement
...
### Story 9.4: AD-004 — User Management — Full CRUD
...
### Story 9.5: AD-005 — Integration Monitoring Dashboard
...
### Story 9.6: AD-006 — Tenant / Clinic Configuration
...
### Story 9.7: AD-007 — Session Management
...
### Story 9.8: AD-008 — System Health Dashboard
...
### Story 9.9: AD-009 — Terminology / Value Set Management
...
### Story 9.10: AD-010 — Data Export and Reporting
...
### Story 9.11: AD-011 — Backup and Restore Procedures
...
### Story 9.12: AD-012 — PHI Encryption at Rest
...
### Story 9.13: AD-013 — Rate Limiting and DDoS Protection
...
### Story 9.14: AD-014 — System-Wide Announcements
...
### Story 9.15: AD-015 — Error Boundaries and Clinical Fail-Safe UI
...
## Epic 10: Billing Specialist — Coding & Revenue Cycle

Enable OHIP/private billing coding, claim submission, rejection management, and revenue cycle reporting from the encounter record.

### Story 10.1: BI-001 — Billing Work Queue
...
### Story 10.2: BI-002 — ICD-10-CA Coding Interface
...
### Story 10.3: BI-003 — OHIP Fee Code Entry and Validation
...
### Story 10.4: BI-004 — OHIP Claim Submission (MC EDT)
...
### Story 10.5: BI-005 — Rejected Claim Management
...
### Story 10.6: BI-006 — Private Insurance and Self-Pay Billing
...
### Story 10.7: BI-007 — Revenue Cycle Analytics
...
### Story 10.8: BI-008 — OHIP Billing Compliance — Time-Barred Claims Alert
...
### Story 10.9: BI-009 — Superbill / Billing Summary from Encounter
...
### Story 10.10: BI-010 — Patient Billing Statement
...
## Epic 11: Cardiac Technician — Procedure & Diagnostic Workflows

Support ECG/Echo/Holter/stress-test workflows with structured results, DICOM integration, and critical finding escalation.

### Story 11.1: TK-001 — ECG Result Entry and Structured Interpretation
...
### Story 11.2: TK-002 — Echo Structured Result Form
...
### Story 11.3: TK-003 — Stress Test Protocol Documentation
...
### Story 11.4: TK-004 — Holter Monitor Assignment and Return
...
### Story 11.5: TK-005 — DICOM Viewer Integration
...
### Story 11.6: TK-006 — Critical Finding Escalation
...
### Story 11.7: TK-007 — Previous Study Comparison
...
### Story 11.8: TK-008 — Technician Workload Management
...
### Story 11.9: TK-009 — FHIR DiagnosticReport Write-Back
...
### Story 11.10: TK-010 — Procedure Room Equipment Status
...
## Epic 12: Cross-Cutting — Platform, Integration & Quality

Deliver shared platform capabilities — real-time notifications, workflow orchestration, FHIR integration, SMART on FHIR, bilingual support, offline mode, and test/performance infrastructure — that underpin every persona epic above.

### Story 12.1: CC-001 — Real-Time WebSocket / SSE Notifications
...
### Story 12.2: CC-002 — Temporal Workflow Engine (Long-Running Processes)
...
### Story 12.3: CC-003 — Patient Demographics Management (Create/Edit Patient FHIR Record)
...
### Story 12.4: CC-004 — Inter-Application FHIR Integration (EHR ↔ LIMS ↔ PharmacyMS)
...
### Story 12.5: CC-005 — SMART on FHIR Authorization Server
...
### Story 12.6: CC-006 — PCR (Provincial Client Registry) Integration
...
### Story 12.7: CC-007 — Bilingual Support (EN/FR) Completion
...
### Story 12.8: CC-008 — Offline / Progressive Web App Mode
...
### Story 12.9: CC-009 — Mobile and Tablet Optimised Layout
...
### Story 12.10: CC-010 — FHIR R4 Profile Validation
...
### Story 12.11: CC-011 — Error Handling and User Feedback Standards
...
### Story 12.12: CC-012 — End-to-End Test Suite (Playwright)
...
### Story 12.13: CC-013 — Unit Test Coverage Targets (Vitest)
...
### Story 12.14: CC-014 — Performance and Load Testing
...
### Story 12.15: CC-015 — Comprehensive Audit Logging Architecture
...
### Story 12.16: CC-016 — FHIR Bulk Data Export ($export)
