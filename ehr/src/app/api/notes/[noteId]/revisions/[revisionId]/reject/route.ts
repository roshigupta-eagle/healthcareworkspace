...
import { getNote, rejectRevision } from "@/notes/service.mock";
...
export async function POST(
  ...
  try {
    const updated = await withAudit(
      { agentId: actor.id, entityType: "NoteRevision", entityId: revisionId, action: "U", description: "Reject tracked change" },
      async () => rejectRevision(noteId, revisionId, actor, body?.opIds)
    );
