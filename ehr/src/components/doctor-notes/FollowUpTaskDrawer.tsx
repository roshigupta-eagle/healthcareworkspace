...
import { formatNoteDate } from './constants';
...
type Props = {
  ...
  onClose: () => void;
  onCreated: (task: ClinicalTask, updatedNote: DoctorNote) => void;
  onCompleted: (task: ClinicalTask) => void;
  onToast: (message: string, level?: 'success' | 'error' | 'info') => void;
};

export default function FollowUpTaskDrawer({ patientId, note, existingTask, onClose, onCreated, onCompleted, onToast }: Props) {
  const [title, setTitle] = useState(`Follow-Up: ${note.type}`);
  const [assignee, setAssignee] = useState(note.author.name);
  ...
  if (existingTask) {
    ...
    return (
      <Drawer title="Follow-Up Task" subtitle={note.type} onClose={onClose}>
        <div className="space-y-3 text-sm text-slate-700">
          <div className="rounded-lg border border-slate-100 bg-slate-50 p-4">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-slate-500">
              ...
              <div>Due: <span className="font-medium text-slate-700">{existingTask.dueDate ? formatNoteDate(existingTask.dueDate) : '—'}</span></div>
              <div>
                Status:{' '}
                <span className={`font-medium ${isComplete ? 'text-emerald-700' : 'text-amber-700'}`}>{isComplete ? 'Completed' : 'Open'}</span>
              </div>
            </div>
          </div>
          {!isComplete && (
            <button
              ...
              onClick={async () => {
                setSubmitting(true);
                try {
                  const res = await fetch(`/api/patients/${encodeURIComponent(patientId)}/notes/follow-up-tasks/${encodeURIComponent(existingTask.id)}`, {
                    method: 'PATCH',
  ...
  return (
    <Drawer
      ...
      subtitle={`${note.type} · ${note.author.name}`}
      onClose={onClose}
      footer={
        <div className="flex justify-end gap-2">
          ...
          <button
            ...
            onClick={async () => {
              setSubmitting(true);
              try {
                const res = await fetch(`/api/patients/${encodeURIComponent(patientId)}/notes/${encodeURIComponent(note.id)}/follow-up`, {
                  method: 'POST',
                  headers: { 'content-type': 'application/json' },
