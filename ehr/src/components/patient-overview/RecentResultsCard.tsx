...
type RecentResultsPatient = {
  readonly id: string;
  readonly labResults?: readonly RecentLabResult[];
};
...
export function RecentResultsCard({ patient }: { patient: RecentResultsPatient }) {
          ...
          {results.slice(0, 4).map((l) => (
            <li key={l.id} className="border-b border-gray-100 last:border-0 pb-2 last:pb-0">
              <Link
                ...
                aria-label={`Open laboratory comparison for ${l.name}`}
                className="block rounded-lg p-2 -m-2 transition hover:bg-teal-50/60 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-teal-100"
              >
